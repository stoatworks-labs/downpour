...
const unsigned int* DownpourPlugin::PresetParamIDsForTest( int& count )
{
	count = downpour::presets::kParamCount;
	return kPresetParamIDs;
}

float DownpourPlugin::presetValue( int presetIndex, unsigned int id ) const
{
	if( presetIndex <= 0 || presetIndex > downpour::presets::kCount )
		return -1.0f;

	const downpour::presets::Preset& preset = downpour::presets::kPresets[ presetIndex - 1 ];
	for( int j = 0; j < downpour::presets::kParamCount; ++j )
...
void DownpourPlugin::seedHostValues()
{
...
bool DownpourPlugin::hostIsRestatingItself( unsigned int index, float value )
{
...
void DownpourPlugin::applyPreset( int presetIndex )
{
...
FFResult DownpourPlugin::SetTime( double time )
{
	hostTimeSeen = true;
	return CFFGLPlugin::SetTime( time );
}

void DownpourPlugin::SetClockScaleForTest( double scale )
{
	clockScale = scale;
}
...
double DownpourPlugin::HostSecondsForTest() const
{
	return hostSeconds;
}
